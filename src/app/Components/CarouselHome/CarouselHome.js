...
export default function CarouselHome() {
    ...
    return (
        <div className="flex flex-row gap-3">
            ...
            <animatedPresence
                mode="wait"
                className="relative w-full bg-gray-300 rounded-b-2xl"
            >
                <motion.div
                    key={currentSlide}
                    initial={{ opacity: 0 }}
                    ...
                    className="lg:h-[400px] w-full bg-cover font-bold rounded-b-2xl object-cover"
                ...
                >
                    ...
                    <div className="text-white flex flex-col gap-3">
                        ...
                        transition={{ duration: 1 }}
                            className="flex flex-col gap-4 md:text-4xl lg:text-5xl"
                        >
                            {slide.title}
                    ...
                    <div className="flex gap-10 text-[var(--pink)]">
                        {slide.links.map((link, index) => (
                            <div
                                className="bg-white font-extrabold p-3.5 rounded-[10px] cursor-pointer shadow-2xl shadow-black hover:scale-[1.1] duration-100"
                                key={index}>
                                <Link href="#">{link}</Link>
                            </div>
                        ))}
